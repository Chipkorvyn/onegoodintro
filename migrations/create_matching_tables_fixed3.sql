...
-- Users can see their own confirmed matches (cast auth.uid() to TEXT)
CREATE POLICY "Users can view their confirmed matches" ON confirmed_matches
  FOR SELECT USING (seeker_id = auth.uid()::TEXT OR helper_id = auth.uid()::TEXT);

CREATE POLICY "Users can update their acceptance" ON confirmed_matches
  FOR UPDATE USING (seeker_id = auth.uid()::TEXT OR helper_id = auth.uid()::TEXT);

-- Users can see their own notifications (cast auth.uid() to TEXT)
